...
1. 寄存器、高速缓存、内存、外存的访问特征？
   - 存储器的位置、容量、访问频率以及存储管理是否需要软件参与
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   - 均衡繁忙是指各个存储介质都处于繁忙状态，但又不构成系统瓶颈
3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
   - 在数组过大或者是递归过深的时候会导致内存不够，此时通常会尝试优化算法例如剪枝或者压缩数组之类的方式。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   - 依据程序逻辑结构，将程序划分为若干功能相对独立的模块；将不会同时执行的模块共享同一块内存区域
   - 程序开发者需要划分功能模块并确定模块间的覆盖关系
2. 什么是交换技术？覆盖与交换有什么不同？
   - 以进程为单位的地址空间换入和换出
   - 覆盖是以模块为单位解决进程空间大于物理空间的问题；交换是以进程为单位解决高优先级进程所需要的内存空间问题
...
4. 如何分析内核模块间的依赖关系？
   - 在Linux系统下可以使用 `depmod` 命令，<http://www.linuxso.com/command/depmod.html>
5. 如何获取内核模块间的函数调用列表？
   - 静态代码分析、动态函数调用跟踪
...
1. 什么是时间局部性、空间局部性和分支局部性？
   - 时间局部性：一个数据/指令的访问执行常集中在一个较短的时间内
   - 空间局部性：一个数据/指令的访问执行常集中在一个较小的区域内
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   - 虚拟内存只将当前指令执行所需要的部分进程地址空间放在物理内存之中
   - 与覆盖的区别：不需要程序员手动分割模块；与交换的区别：虚拟内存换出时，只需要移动部分的地址空间，不用将整个进程换出
   - 好处：可以提供用户更大的可使用地址空间、不同进程之间独占虚拟内存，互不干扰；挑战：部分交换和不连续的特征需要较为复杂的技术支持、机制受局部性原理约束
2. 虚拟存储需要什么样的支持技术？
   - 硬件：MMU、操作系统：置换算法
...
### 8.5 虚拟页式存储
 ...
 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
    ...
    - 增加一个驻留位表示是否在内存
      ...
      > 虚拟页表项的标志位：驻留位表示是否在内存、修改位表示是否被修改过、访问位表示是否被访问过、保护位表示允许页面访问方式
...
1. 缺页异常的处理流程？
   ...
   - 查找在外存中的页面、换入页面、修改页表项

   A. 在内存中有空闲物理页面时，分配一物理页面，转第 E 步；

   B. 依据页面置换算法选择将被替换，的物理页帧 f， 对应逻辑页 q；
...
2. 虚拟页式存储管理中有效存储访问时间是如何计算的？
   ...
   EAT = 访存时间 $\times$ (1 - p) + 缺页异常处理时间 $\times$ 缺页率 p
